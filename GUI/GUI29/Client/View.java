...
public class View extends Controller {
	...
	private double oldX, oldY, oldX2, oldY2;
	MainScene ms;
	Stage stage;
	...
	public View(MainScene ms, Stage stage){
		...
		m = new Model(rec, rec2, cir, bounds, bounds2);
	...
	private void actionRec2() {
		rec2.setOnMousePressed(e -> {
			oldX2 = rec2.getLayoutX()-e.getSceneX();
			oldY2 = rec2.getLayoutY()-e.getSceneY();
			
		});
		rec2.setOnMouseDragged(e -> {
			System.out.println("REX\t"+rec.getLayoutX());
			rec2.setLayoutX(oldX2+e.getSceneX());
			rec2.setLayoutY(oldY2+e.getSceneY());
		}); 
	}
